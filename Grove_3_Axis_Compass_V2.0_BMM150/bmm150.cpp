...
//MJS Adds
//========================================================
void BMM150::set_DRDY_bit(uint8_t enable_bit){
	uint8_t reg_data = 0;
	if(enable_bit == 1){
		reg_data = 0x85;
	} else {
	...
	i2c_write(0x4E, reg_data);
}

void BMM150::set_mag_rate(uint8_t rate){
    uint8_t reg_data;

    reg_data = i2c_read(BMM150_OP_MODE_ADDR);
    /*Set the ODR value */
		reg_data = BMM150_SET_BITS(reg_data, BMM150_ODR, rate);
    i2c_write(BMM150_OP_MODE_ADDR, reg_data);
}
